...
class ThreadLocalRecorder {
 public:
  ...
  std::deque<TraceMeRecorder::Event> Consume(
      IncompleteEventTracker* incomplete_event_tracker) TF_MUST_USE_RESULT {
    std::deque<TraceMeRecorder::Event> events;
    std::optional<TraceMeRecorder::Event> event;
    while ((event = queue_.Pop())) {
      if (event->IsStart()) {
        incomplete_event_tracker->AddStart(*std::move(event));
        continue;
      }
      events.push_back(*std::move(event));
      if (events.back().IsEnd()) {
        incomplete_event_tracker->AddEnd(&events.back());
      }
    }
    return events;
  }
...
/* static */ bool TraceMeRecorder::Start(int level) {
  bool started = trace_level_.TryStart(std::max(0, level));
  if (started) {
...
/* static */ TraceMeRecorder::Events TraceMeRecorder::Stop() {
  ...
  if (trace_level_.TryStop()) {
    IncompleteEventTracker incomplete_event_tracker;
    auto recorders = PerThread<ThreadLocalRecorder>::StopRecording();
    result.reserve(recorders.size());
    for (auto& recorder : recorders) {
      auto events = recorder->Consume(&incomplete_event_tracker);
      if (!events.empty()) {
        result.push_back({recorder->Info(), std::move(events)});
      }
    };
    incomplete_event_tracker.HandleCrossThreadEvents();
  }
  return result;
}
