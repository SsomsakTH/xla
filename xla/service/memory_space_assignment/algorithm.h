...
class MsaAlgorithm : public GlobalDecreasingSizeBestFitHeap<HloValue> {
 ...
 private:
  ...
  // Iterates over joint-processed values and creates the allocation values and
  // colocated intervals for all of them. The colocated intervals are created in
  // separate vectors for each joint-processed value to avoid introducing extra
  // colocation constraints across different joint-processed values.
  ...
  // Set of sync copy instructions that we failed/succeeded to replace with
  // asynchronous copies.
